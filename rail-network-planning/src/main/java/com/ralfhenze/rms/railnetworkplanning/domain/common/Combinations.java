...
public class Combinations {
    ...
    public <T> ImmutableList<Pair<T, T>> getUniqueCombinations(ImmutableSet<T> elements) {

        final MutableList<Pair<T, T>> uniqueCombinations = Lists.mutable.empty();
        final Deque<T> sourceElements = new LinkedList<>(elements.castToSet());
        final Deque<T> otherElements = new LinkedList<>(elements.castToSet());
